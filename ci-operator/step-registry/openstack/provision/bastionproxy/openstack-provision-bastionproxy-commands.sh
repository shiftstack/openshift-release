...
if [[ "${CONFIG_TYPE}" != "fipless" ]]; then
    echo "Skipping step due to CONFIG_TYPE not being fipless."
    exit 0
fi
...
EOF
#$SCP_CMD $WORK_DIR/deploy_squid.sh $BASTION_USER@$bastion_fip:/tmp >/dev/null
#$SSH_CMD chmod +x /tmp/deploy_squid.sh >/dev/null
